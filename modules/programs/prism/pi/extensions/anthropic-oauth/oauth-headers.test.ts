...
describe("getUserAgent", () => {
  it("returns claude-cli/<version> (external, sdk-cli) when env is unset", () => {
    ...
    try {
      ...
      // Pinned literal, not just the interpolation: the literal mirrors
      // `ccVersion`. The Anthropic API returns HTTP 400
      // `claude_code_version_too_old` for Fable 5.1 on subscription auth
      // below 2.1.251 (issue #2918).
