...
async function main() {
  ...
  // ── Pass 2: bundled models pi.nix used to hand-roll (issue #2918) ────────
  //
  // pi.nix declared these in models.json until pi bundled them. Check the
  // bundled entries keep what those declarations existed for: without the
  // xhigh map, xhigh degrades to effort "high" (#2053); without
  // forceAdaptiveThinking, requests take the legacy budget form (#2044).
  for (const id of FORMERLY_DECLARED_MODEL_IDS) {
    const entries = models.filter((m) => m.id === id)
    if (entries.length !== 1) {
      throw new Error(
    ...
    const [model] = entries
    if (model.thinkingLevelMap?.xhigh !== "xhigh") {
      throw new Error(
        `expected ${id} thinkingLevelMap.xhigh to be "xhigh", got ` +
          JSON.stringify(model.thinkingLevelMap),
      )
    }
    if (model.compat?.forceAdaptiveThinking !== true) {
      throw new Error(
        `expected ${id} compat.forceAdaptiveThinking to be true, got ` + JSON.stringify(model.compat),
      )
    }
  }
  ...
  console.log(
    `OK: anthropic-oauth extension loaded against ${path.basename(installRoot)}; ` +
      ...
      `bundled ${FORMERLY_DECLARED_MODEL_IDS.join(", ")} carry xhigh + adaptive thinking.`,
  )
}
