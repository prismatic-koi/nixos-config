...
- **review-goal** — whether the change satisfies acceptance criteria or solves the right problem. A feature can be correctly implemented from a security standpoint even if it doesn't meet a requirement — that is review-goal's concern.
...
All security issues are at minimum MAJOR. Issues that can lead to data breach, authentication bypass, or remote code execution are CRITICAL.
