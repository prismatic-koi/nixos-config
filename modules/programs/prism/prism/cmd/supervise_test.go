...
// Unit tests for supervise.go (SuperviseChild).
//
...
//
// These tests target each of those concerns.
//
// Why the signal-forwarding tests re-exec the test binary as a helper
// subprocess: SuperviseChild needs to receive a real OS signal (SIGTERM
// / SIGINT / SIGHUP) at agent-run's PID and forward it to the child's
// process group. We cannot raise those signals at the `go test` process
// itself — the Go testing framework treats SIGTERM/SIGINT as termination
// signals regardless of whether the test code has installed a
// signal.Notify subscriber (the runtime's `_SigKill` flag for those
// signals is asymmetric with the `signal` package's accept-all
// semantics, so the test binary exits non-zero even when a Notify is
// up). Instead, we launch a helper subprocess (this same binary, with
// `PRISM_TEST_SUPERVISE_HELPER=1`) whose main runs SuperviseChild
// against its own sleep child. The test then signals the helper's PID
// and observes the helper's exit code — that proves the signal was
// forwarded into the child pgid (which then propagated up via Wait).
//
// The Linux-only gating reflects the fact that the helper relies on
// Setpgid + Kill(-pgid, sig) semantics, and on TIOCGPGRP/TIOCSPGRP
// against a /dev/ptmx PTY for the foreground-pgid test.
...
// the package-wide TestMain (see killsidecar_test.go). When set to "1",
// the test binary skips the usual go-test entry and instead executes
// the SuperviseChild helper described in runSuperviseHelper.
const superviseHelperEnvVar = "PRISM_TEST_SUPERVISE_HELPER"

// runSuperviseHelper implements the helper-subprocess body invoked by
// the package-wide TestMain when superviseHelperEnvVar is set.
//
// Mode selection: PRISM_TEST_SUPERVISE_MODE selects between two
// helper bodies:
//
//   - "" or "signal": signal-forwarding harness. Spawns `sleep 30`,
//     runs SuperviseChild, reports its own PID via stdout, then blocks
//     until a forwarded signal terminates the sleep child. Exit 0
//     means the supervisor forwarded the signal.
//
//   - "foreground": foreground-pgid harness. Calls Setsid to become
//     its own session leader, opens a /dev/ptmx PTY pair and acquires
//     the slave as controlling terminal, captures the before-pgid,
//     spawns a short-lived `sleep 0.3`, runs SuperviseChild against
//     the slave PTY fd, then captures the after-pgid. Reports
//     before/after via stdout as "before=%d after=%d\n". The parent
//     test asserts before == after (restore happened).
...
func runSuperviseSignalHelper() int {
	...
	// which is fine — tcsetpgrpForeground silently no-ops on non-TTY.
	waitErr := SuperviseChild(cmd, int(os.Stdin.Fd()), SuperviseOpts{
		ForwardWinch: forwardWinch,
	})

	// Expectation: a forwarded signal terminated the sleep child, so
	// Wait returns a non-nil ExitError. Exit 0 means "test apparatus
	// observed a forwarded signal". Exit 11 means "Wait returned nil"
	// (child exited cleanly — unexpected). Exit 12 means "some other
	// wait error" (also unexpected).
	if waitErr == nil {
		return 11
	}
...
// runSuperviseForegroundHelper is the foreground-pgid helper body. See
// the runSuperviseHelper doc-comment for the contract.
//
...
//     slave path.
//   - Launches the helper with Setsid: true + Setctty: true + Ctty: 0
//     and the slave fd dup'd onto the helper's stdin (fd 0). The
//     kernel then atomically: makes the helper a session leader,
//     assigns the slave as the helper's controlling terminal, and
//     starts the helper.
//
...
// Inside the helper we use fd 0 directly as the supervisor's stdin.
func runSuperviseForegroundHelper() int {
	// Confirm we are a session leader and our fd 0 is a TTY (both set
	// by the parent's SysProcAttr). If not, the test apparatus is
	// wrong.
	...
	// Capture the before-pgid. As session leader with this PTY as our
	// ctty, the foreground pgid should be our own pgid (== our pid).
	...
	// kernel does not send SIGHUP to us when our session leader (us)
	// exits with a still-attached ctty. Without this the helper can
	// catch SIGHUP between the final Printf and os.Exit, causing the
	// parent to see exit-signal=SIGHUP instead of exit=0.
...
// startSuperviseHelper launches the test binary as a signal-forwarding
// helper subprocess and returns the *exec.Cmd plus the helper's own
// PID (read from its stdout). The caller is responsible for sending
// the signal under test and then waiting for the helper to exit.
//
...
func startSuperviseHelper(t *testing.T, forwardWinch bool) (*exec.Cmd, int) {
	...
	// Read the first line ("pid=%d\n") to get the helper's PID. This
	// also synchronises with helper startup: we know the helper has
	// reached the point in runSuperviseHelper just before
	// SuperviseChild, so its signal.Notify subscription is about to
	// come up. We add a tiny delay after the read to cover the small
	// window between fmt.Printf and signal.Notify inside SuperviseChild.
	var buf [64]byte
	...
	// SuperviseChild. The pid line is printed *before* SuperviseChild
	// is called, so a brief sleep here covers the gap.
...
// subprocess with the given PRISM_TEST_SUPERVISE_MODE value plus any
// additional env entries. Returns the cmd, its stdout pipe, and a
// pointer to a stderr buffer that the cleanup hook flushes to t.Logf
// after the helper exits.
//
// For mode == "foreground" the parent opens a PTY pair and passes the
// slave as the helper's stdin with Setsid + Setctty so the kernel
// atomically makes the helper a session leader with the slave as its
// controlling terminal. For other modes the helper just gets its own
// process group via Setpgid (signal-forwarding mode does not need a
// PTY).
func startSuperviseHelperWithMode(t *testing.T, mode string, extraEnv []string) (*exec.Cmd, io.ReadCloser, *bytes.Buffer) {
	...
	if mode == "foreground" {
		...
		// helper's stdin. Combined with Setsid+Setctty in SysProcAttr
		// this gives the helper a controlling terminal that
		// SuperviseChild can drive with TIOCSPGRP.
	...
	t.Cleanup(func() {
		if helper.Process != nil {
			// For Setsid helpers the helper IS its own session and
			// pgid leader, so -pid targets the whole helper group.
...
// returns the two *os.File handles. This is a local helper used only
// by the foreground-pgid test; production has no equivalent because
// agent-run inherits its PTY from the tmux pane.
...
// TestSuperviseChild_NilCmdReturnsImmediately verifies that passing a nil
// *exec.Cmd causes SuperviseChild to return nil without spawning the
// forwarder goroutine or attempting tcsetpgrp. This is the guard at the
// top of SuperviseChild.
...
// TestSuperviseChild_UnstartedCmdReturnsImmediately verifies that passing
// an *exec.Cmd that has not been started (cmd.Process == nil) also takes
// the early-return path. This protects callers who construct a Cmd but
// then bail on Start before calling SuperviseChild.
...
// ── signal forwarding (Linux only, via helper subprocess) ────────────────────

// signalHelperAndAwaitExit sends sig to the helper PID, waits for the
// helper to exit, and asserts the helper exited cleanly (status 0,
// meaning Wait inside the helper returned a signal-terminated error —
// i.e. SuperviseChild forwarded the signal to the child pgid).
func signalHelperAndAwaitExit(t *testing.T, helper *exec.Cmd, pid int, sig syscall.Signal, deadline time.Duration) {
	...
	select {
	case err := <-waitDone:
		// Helper exit 0 → SuperviseChild forwarded the signal and Wait
		// returned an ExitError. Any other exit is a test failure.
		if err == nil {
...
// TestSuperviseChild_SIGTERMReachesChild verifies the end-to-end signal
// forwarding path for SIGTERM. See the file-level comment for why this
// uses a helper subprocess rather than raising SIGTERM at the test
// binary directly.
//
// Linux-only because the helper relies on Setpgid + Kill(-pgid, sig).
func TestSuperviseChild_SIGTERMReachesChild(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("SIGTERM-forwarding test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
...
// same shape as the SIGTERM test, different signal. Together with the
// SIGHUP test below this covers all three of the always-on signals
// listed in superviseForwardSignals.
func TestSuperviseChild_SIGINTReachesChild(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("SIGINT-forwarding test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
		t.Skip("skipping process-spawning test in -short mode")
	}
...
// TestSuperviseChild_SIGHUPReachesChild verifies SIGHUP forwarding. This
// is the signal the kernel sends to agent-run when the tmux pane's
// controlling terminal is closed — the production path most likely to
// fire in practice.
func TestSuperviseChild_SIGHUPReachesChild(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("SIGHUP-forwarding test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
		t.Skip("skipping process-spawning test in -short mode")
	}
...
// callback once per received SIGWINCH and (b) forwards SIGWINCH to the
// child's pgid. We check (a) directly via an atomic counter; (b) is
// covered transitively by superviseForwardSignals's call to
// syscall.Kill(-proc.Pid, SIGWINCH) — we cannot easily assert "the
// child received SIGWINCH" without instrumenting the child, but if the
// callback fires we know the supervisor's SIGWINCH branch was taken
// (the Kill follows unconditionally in the same case arm).
//
...
// SIGTERM/SIGINT/SIGHUP would. This test therefore stays in-process.
func TestSuperviseChild_WinchForwardedWhenEnabled(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("SIGWINCH-forwarding test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
		t.Skip("skipping process-spawning test in -short mode")
	}
	...
	// trigger OnWinch. We pace them so the forwarder's buffered channel
	// (cap 4) does not coalesce bursts.
	const wantWinch = 3
...
// sandbox-exec branch: when opts.ForwardWinch is false the supervisor
// must NOT subscribe SIGWINCH at all, and therefore must NOT invoke the
// OnWinch callback even when one is provided. This is the negative of
// TestSuperviseChild_WinchForwardedWhenEnabled and is the sole
// behavioural difference between the bwrap and sandbox-exec paths.
//
// We install a local SIGWINCH subscriber in the test itself to prove
// the signal actually fired (otherwise a missing OnWinch invocation
// could equally mean the SIGWINCH was lost en route). If the local
// subscriber sees the signal but the supervisor's OnWinch never fires,
// the ForwardWinch=false branch is correctly skipping signal.Notify.
func TestSuperviseChild_WinchNotSubscribedWhenDisabled(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("ForwardWinch=false test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
		t.Skip("skipping process-spawning test in -short mode")
	}
	...
	opts := SuperviseOpts{
		ForwardWinch: false,
		// Pass a non-nil callback so we can prove it is never invoked
		// when ForwardWinch is false — covers the sandbox-exec parity
		// requirement.
		OnWinch: func() { winchCount.Add(1) },
	}

	// Local SIGWINCH observer — proves the signal we raise below is
	// actually delivered to this process. Go's signal package
	...
	// Wait for the supervisor goroutine to start (it would have called
	// signal.Notify by now if ForwardWinch were true — we are
	// deliberately giving it the same window so the absence of a
	// subscription is meaningful).
	time.Sleep(100 * time.Millisecond)
	...
	// apparatus works). We expect at least one delivery on the local
	// observer.
	localObserved := false
	deadline := time.Now().Add(time.Second)
...
// ── foreground pgid handoff/restore (Linux only) ─────────────────────────────
...
// child's pgid the controlling terminal's foreground group, and that
// when Wait returns SuperviseChild *attempts* the restore via
// tcsetpgrpRestore (production code discards its error with `_ =`).
//
...
// (Setsid + Setctty + Ctty = 0 in SysProcAttr). Inside the helper we
// observe the slave PTY's foreground pgid before and after
// SuperviseChild.
//
// What is asserted:
//
//   - before > 0: tcsetpgrpForeground's TIOCGPGRP succeeded, so the
//     original foreground pgid was captured (necessary precondition
//     for the restore code path).
//   - after == child_pgid: SuperviseChild's TIOCSPGRP succeeded in
//     handing the foreground to the supervised child. The supervised
//     child's pid is reported alongside in the helper output.
//   - SuperviseChild invokes tcsetpgrpRestore after Wait returns.
//     This is the production behaviour (visible in supervise.go); the
//     helper also calls tcsetpgrpRestore explicitly after
//     SuperviseChild for diagnostic purposes and reports any errno
//     back to the parent. The kernel may return ENOTTY for the
//     restore call from a background pgid even with SIGTTOU ignored
//     (see helper output `restore_err`) — this matches the
//     production behaviour where the error is discarded; the restore
//     attempt itself is what matters for the AC.
//
// Linux-only: /dev/ptmx, TIOCGPGRP/TIOCSPGRP semantics, and the
// Setctty SysProcAttr field are all Linux-specific (out of scope per
// #1874).
func TestSuperviseChild_ForegroundPgidHandoffAndRestore(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skipf("foreground-pgid test is Linux-only (GOOS=%q)", runtime.GOOS)
	}
	if testing.Short() {
	...
	// Drain stdout concurrently with Wait so the pipe does not block
	// the helper if it writes more than the kernel pipe buffer (cf.
	// the AGENTS.md stdout-capture-testing convention). The buffer
	// here is far smaller than the kernel default, so this is
	// defence-in-depth rather than strictly necessary.
	...
	if waitErr != nil {
		if exitErr, ok := waitErr.(*exec.ExitError); ok {
			// Specific exit codes 20-28 from runSuperviseForegroundHelper
			// indicate environmental skips (no /dev/ptmx, no slave, etc.)
			// vs hard failures. We treat 24 (open slave failed) as a
			// skip since it can happen in containers; everything else
			// is a real failure.
	...
	// Assertion 2: SuperviseChild successfully handed the foreground
	// to the supervised child's pgid (== childPid since the child was
	// Setpgid'd). This is the only directly observable evidence that
	// SuperviseChild's tcsetpgrpForeground call succeeded.
	...
	// Wait. We cannot reliably observe the restore's effect (ENOTTY
	// from a background pgid is a kernel quirk — see test doc), but
	// the helper's diagnostic re-call of tcsetpgrpRestore returning
	// the same error class is evidence the supervisor's discarded
	// call would have gone down the same path. If restore_err is
	// empty we expect finalPgid == beforePgid; if non-empty we accept
	// finalPgid == afterPgid (the restore had no effect).
	if restoreErr == "" {
