...
// repo_attribution_test.go — issue #2658.
//
// Several code paths write agent_status.repo. Before #2658, three of them held
// their own copy of the repo grammar and all three split on "@" alone, so a
// descendant of a NON-worktree parent was attributed to a repo of its own:
// `obsidian~investigate-v2` instead of `obsidian`. That is the evidence row in
// the issue.
//
// The copy in cmd/sidecar.go was the damaging one. `db.UpsertStatus` sets
...
// the sidecar is the continuous LAST writer. internal/review seeded the
// correct value and the sidecar overwrote it milliseconds later. A test that
...
//     rule — which is exactly how the first draft of this file passed against
//     a reverted cmd/sidecar.go.
...
// TestStoredRepoSurvivesTheSidecarUpsert is the regression test for the
// round-1 blocking finding: the stored repo, after the sidecar has written it,
// must still attribute a descendant to its parent's repo.
//
...
func TestStoredRepoSurvivesTheSidecarUpsert(t *testing.T) {
	...
	cases := []struct {
		...
		// The #2658 shapes: a non-worktree parent and its descendants.
		...
		// whole — the round-1 review caught a draft that truncated it.
...
//
// This is the defect class #2658 exists to close, and the round-1 review found
// two fresh instances of it — cmd/sidecar.go and cmd/switch.go — each silently
// overwriting the correct value at runtime.
//
// # Scope
//
