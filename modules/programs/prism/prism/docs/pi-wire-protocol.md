...
<!-- doclint-skip-file: identifiers | this document specifies the wire protocol between the prism sidecar and the external pi coding-agent. Most backticked identifiers here (`RpcCommand`, `modelId`, TypeScript field names, snake_case RPC message types) live in the external pi package and cannot be resolved by the local doclint. The prose is still scanned for ASD-STE100 (issue #2497). -->
...
P2.EXTENSION (TypeScript, PI extension side). Also reviewers verifying that
the two implementations agree on the wire.

## 1. Architecture overview

prism integrates PI as a harness using a **bidirectional Unix socket**
(Linux) / **localhost TCP listener** (Darwin). This is separate from the
existing host-API socket that already carries `prism spawn`/`prism
prompt`/`prism cleanup` calls from inside the sandbox. The new socket
carries the harness event/control protocol — PI's hook events flow out, and prism's prompt/model
control flows in.
...
This document specifies framing rules (§3), the transport layer (§2), the
version handshake (§4), and the frame catalogues in both directions (§5,
§6). It also covers failure modes (§7), forward-compat and version 2
extension rules (§8), and cross-references to existing prism subsystems
(§9).
...
- TCP listener on `127.0.0.1:<harness_port>`. The listener binds on
  ...
  reached it from the container VM — `sandbox-exec` runs **directly on
...
- Failure to bind is **fatal**. Silent fallback reproduces the
...
at [`packages/coding-agent/docs/rpc.md`][pi-rpc] in the PI monorepo. That
convention explicitly notes that Unicode line separators (U+2028, U+2029)
are **not** record separators because they appear inside valid JSON
strings. Any client
...
- `protocol_version` (integer, required) — the wire protocol version the
  extension speaks. Current version is **2** (bumped from 1 in #1434. The
...
- `harness_version` (string, required) — the PI package version (for
  example `"0.66.1"`). Used for diagnostic logging only in v1. The sidecar does
...
- `session_name` (string, required) — the prism session name (for
  example `"<repo>@<branch>"`). The extension surfaces this in PI's session-name
  display (§3.5 of #1210) so the user sees the prism context.
- `session_role` (string, required) — `"worker"` or `"coordinator"`,
  derived from the sidecar's `Config.AgentRole`. The extension can
  branch on this to adjust behaviour (for example, coordinators do not
  auto-start the merge-queue watcher from inside PI — that lives in the
  sidecar).
...
- The handshake is complete in one round-trip — there is no separate
  capability-negotiation step. v1 has no capability flags. v2 can add
...
- `state` (string, required) — one of:
  ...
  - `"waiting"` — agent is waiting on out-of-band input (for example a
    permission prompt or extension UI dialog). Extension emits this when
...
from a pre-v2 extension rejects the connection at the
`hello_ack` handshake due to the protocol version mismatch. No
silent state corruption occurs as a result.
...
- `parentMessageId` (string, optional) — the `messageId` of the assistant
  ...
  yet observed an assistant `message_start` in this session — for
  example, an extension that restarted mid-turn. Issue #1787 added this field so the
  consumer-side secondary-query SQL pushdown
  (`db.QueryEventsByMessageIDs`, used by `prism checkin --turns`)
  can pair tool events back to their parent
...
- `parentMessageId` (string, optional) — mirrors the field on `tool_call`
  ...
  matching tool call. Stamped by the extension, with the same omission
  semantics as on `tool_call`. Both frames carry the same value, so the consumer's
...
- `text` (string, required) — the delta text. Can be empty (treated as a
  zero-length fragment that contributes nothing to the coalesced text).
...
zero/absent for non-cache-eligible providers). The sidecar persists whatever arrives
...
start of each turn (`turn_start`). This way the sidecar always has an
up-to-date picture of the session state.
...
- `review_cycles` (integer, required) — current review-cycle count for the
  detected PR. Zero when no PR is detected yet.
...
- `session_id` (string, required) — the PI session ID from
  ...
  archiving. Can be `""` in the post-handshake frame when no turn has
...
tool execution is in flight. This keeps long-running invocations such as
`nix build .#prism` or `go test -count=20` from silencing the wire long
...
- `name` (string, required) — the tool name (for example `"bash"`). Informational
...
throw propagates into the dispatcher's outer try/catch and silently
drops the frame. The extension therefore queries `ctx.isIdle()` at delivery
time and routes:
...
send `deliver_as: "followUp"` explicitly (which is what
...
scope, retry-on-failure, and more — lives in the sidecar (P3.LIVE in a
later issue), not in this protocol. From this document's perspective the
sidecar decides. The extension obeys.

### 6.4 `register_provider`

Register a provider configuration with PI's runtime. This matches PI's
...
its `pendingReviewCall` guard. That guard suppresses `state_change:finished`
on turn boundaries while a `prism review` is in flight.
...
- `in_flight` (bool, required) — `true` while the sidecar is awaiting a
  ...
  review is delivered (or never started).
...
- Whenever the in-memory `reviewingInFlight` flag transitions to a new
  ...
  - `flushPendingReplay` clearing it to `false` after a buffered
    review-complete delivery is re-enqueued post-reconnect.
...
latch). This is the secure-by-default direction: a missed `in_flight:
true` produces an extra notification at worst, while a missed `in_flight:
false` strands the session as `active` (the #2050 failure shape).
...
and extension ship together via the same Nix package. The version-skew
window therefore exists only on a paused/resumed session that crosses a
prism upgrade boundary.
...
5 minutes, the same default as bwrap-mode opencode in
...
command. That command triggers a `session_start` event in the extension
and causes the old socket connection to close before the extension opens
a new one.
...
- Log the error with the first 200 bytes of the offending line truncated
  and base64-encoded so it cannot corrupt log streams.
...
syntactically valid but semantically wrong. For example, the extension
sent a `state_change` before `hello`, or `hello.protocol_version` is not
1, and other similar cases.
...
- The extension's outbound queue is the natural backpressure point of
  ...
  in-kernel buffer is full and the application must pause. The
  extension must not call `socket.write` faster than the kernel
...
It treats the stall as §7.2 (connection dropped).
...
into prism's snake_case wire field names — for example `toolCallId` → `id`,
...
`ArchiveAdapter.Export` interface method was **removed** along with
opencode itself (issue #2185). PI is the only remaining harness and its
on-disk JSONL is already pi-mono-v3 shaped. The archive pipeline
therefore writes `session.jsonl` directly into the per-session archive
directory in a single step. There is no separate Export normalisation
pass, and the pre-fix `raw/session.jsonl` indirection is gone. If a future harness
...
- **New optional fields** can be added to existing frame types in v2.
...
- **New frame types** can be added in v2. Both sides must tolerate
...
- **Capability flags** can be added as optional fields on `hello`/
...
- **Deprecation path:** when v3 deprecates a v1 field, the sidecar at
  ...
  line. v4 can remove support after at least one full release cycle
...
- The host-API socket pattern in
  ...
  same Darwin TCP-fallback pattern. P2.SIDECAR must add
...
- The sidecar's own state machine internals (finished-debounce timing,
  reconnect-after-crash policy beyond §7.6, and more) — these are
...
- The exact TypeScript class structure of the PI extension — owned
  by P2.EXTENSION.
- Multi-connection support (for example a "dashboard tail" client connecting
