...
- PI session storage location is the **same in every mode**: the host PI sessions root — `$PI_CODING_AGENT_DIR/sessions/` when set, else `~/.pi/agent/sessions/` — for `bwrap` via an overlay mount into the sandbox, for `sandbox-exec` via the injected `PI_CODING_AGENT_DIR` env var (the sandbox shares the host filesystem; #2210). The root is shared across any PI invocation on the same `cwd`, **not owned by prism**, and survives `prism cleanup` — only the cleaned session's own transcript is removed, by the #2035 resume-linkage sever.
