...
- The three lifecycle clears — `harness_port` release, `harness_session_id` clear, and `ended_at` stamp — run regardless of whether the worktree path was found and regardless of whether the tmux session was still alive at cleanup time; they also run for sessions without a worktree (review subsessions named `<parent>~review-<N>-<agent>`).
- Re-running `prism cleanup --yes --session <name>` on an already-ended session is idempotent: each of the three lifecycle updates is a no-op success, the `ended_at` timestamp is not re-stamped (the `WHERE ended_at IS NULL` guard suppresses the UPDATE), and the command exits 0.
...
- The PI JSONL transcript copied into the archive is **also retained** on disk after `prism cleanup` for `host` and `bwrap` sessions (the source lives under `<piSessionsRoot>/<encoded-cwd>/` — `$PI_CODING_AGENT_DIR/sessions/` when the env var is set, else `~/.pi/agent/sessions/` — which is not owned by prism); for `sandbox-exec` sessions the original transcript is removed when the per-instance staging `HOME` is deleted, and only the archive copy remains.
...
- The PI sessions root differs by isolation mode: `host` and `bwrap` both resolve to `$PI_CODING_AGENT_DIR/sessions/` when the env var is set on the prism CLI's environment, else `~/.pi/agent/sessions/` (for `bwrap`, the directory is overlay-mounted into the sandbox at `$PI_CODING_AGENT_DIR/sessions/`); `sandbox-exec` resolves to `<staging-home>/.pi/agent/sessions/` regardless of the host's `PI_CODING_AGENT_DIR`. The env-var lookup mirrors pi's own `ENV_AGENT_DIR` honouring (pi 0.78 `dist/core/session-manager.js`). For `host` and `bwrap`, the `<encoded-cwd>` directory is shared across any prism session whose worktree matches the cwd; for `sandbox-exec`, the JSONL lives under the per-instance staging `HOME` and is not shared.
...
- PI session storage location differs by mode: `host` and `bwrap` both write to the host PI sessions root — `$PI_CODING_AGENT_DIR/sessions/` when set, else `~/.pi/agent/sessions/` — (for `bwrap`, via an overlay mount into the sandbox), shared across any PI invocation on the same `cwd` and **not owned by prism** — survives `prism cleanup`; `sandbox-exec` writes to the per-instance staging `HOME` and is removed by `prism cleanup`.
