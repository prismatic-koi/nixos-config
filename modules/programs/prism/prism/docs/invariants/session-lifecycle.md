...
- When a non-empty `--prompt` is supplied, the prompt body is written to the per-session run directory and delivered to the agent by file path (`--prompt "$(cat <path>)"` in host mode or `PRISM_INITIAL_PROMPT_FILE=<path>` in `bwrap`/`sandbox-exec` mode), never inlined into the tmux launch argv.
...
- `prism spawn` enforces a per-isolator concurrency cap (default `6`) and refuses to start a new session over the cap unless `--ignore-concurrency-cap` is passed.
...
- The PI sessions root differs by isolation mode: `host` → `~/.pi/agent/sessions/`, `sandbox-exec` → `<staging-home>/.pi/agent/sessions/`, `bwrap` → `$XDG_STATE_HOME/prism/run/<sessionDirHash>/pi-agent/sessions/`. For `host` and `sandbox-exec`, the `<encoded-cwd>` directory is shared across any prism session whose worktree matches the cwd; for `bwrap`, the JSONL lives under the per-session run directory and is not shared.
...
- A **worker** is any non-coordinator session created by `prism spawn` (typically with `root_agent_name = 'worker'`). Workers may run `prism review`, `prism escalate`, and `prism investigate` is denied in their bash deny list along with `prism merge`.
...
- PI session storage location differs by mode: `bwrap` → per-session run dir (not shared across sessions); `sandbox-exec` → per-instance staging `HOME` (cleaned up on `prism cleanup`); `host` → conventional `~/.pi/agent/sessions/`, **not owned by prism**, survives `prism cleanup`, and is shared with any other PI invocation on the same `cwd`.
...
- The concurrency cap is per-isolator: `bwrap`, `sandbox-exec`, and `host` each enforce their own cap independently via `Isolator.Cap`.
