...
func TestEscalatedState(t *testing.T) {
	tests := []struct {
		...
		{"all 5 finished",
			...
			"finished"},
		// compacting is a first-class state — must not render as idle.
		{"single compacting", []string{"compacting"}, "compacting"},
		{"compacting beats interrupted", []string{"interrupted", "compacting"}, "compacting"},
		{"compacting beats finished", []string{"finished", "compacting"}, "compacting"},
		{"active beats compacting", []string{"compacting", "active"}, "active"},
		{"error beats compacting", []string{"compacting", "error"}, "error"},
		{"waiting beats compacting", []string{"compacting", "waiting"}, "waiting"},
		// Full precedence table (waiting > error > active > compacting > interrupted > finished > idle).
		{"precedence: waiting first",
			[]string{"finished", "interrupted", "compacting", "active", "error", "waiting"},
			"waiting"},
		{"precedence: error first when no waiting",
			[]string{"finished", "interrupted", "compacting", "active", "error"},
			"error"},
		{"precedence: active first when no waiting/error",
			[]string{"finished", "interrupted", "compacting", "active"},
			"active"},
		{"precedence: compacting beats interrupted",
			[]string{"finished", "interrupted", "compacting"},
			"compacting"},
