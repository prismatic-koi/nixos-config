...
type Config struct {
	...
	// PISystemPromptHostPath is the absolute host path to the per-session
	// system-prompt file generated by WriteSystemPromptFile. When non-empty
	// and Harness == "pi", BuildArgs (bwrap) bind-mounts this file read-only
	// into the sandbox at PISystemPromptSandboxPath and passes it to PI via
	// --append-system-prompt.
	PISystemPromptHostPath string

	// PISystemPromptSandboxPath is the in-sandbox path at which the system
	// prompt file is accessible. Defaults to /tmp/prism-system-prompt.md when
	// empty (set by WriteSystemPromptFile).
	PISystemPromptSandboxPath string
