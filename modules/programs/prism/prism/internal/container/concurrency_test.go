...
// capFromDB is a helper that exercises the same logic as an isolator.Cap()
// call by calling ListInFlight directly and constructing a CapStatus.
func capFromDB(t *testing.T, dbPath string, ps func() ([]string, bool)) CapStatus {
	t.Helper()
	inFlight, psFailed := ListInFlight(dbPath, ps)
	limit := DefaultConcurrencyCap
	count := len(inFlight)
	note := ""
	if psFailed {
		note = "ps failed — concurrency check is using DB-only count (may be imprecise)"
	}
	return CapStatus{
...
func TestCap_FiveSessionsAllows(t *testing.T) {
	...
	status := capFromDB(t, path, emptyPodman)
	if status.Count != 5 {
