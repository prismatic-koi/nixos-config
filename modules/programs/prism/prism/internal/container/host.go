...
// are no-ops. AgentRun returns an error because host mode reaches the agent
...
// BuildRunArgs is a no-op for host mode: the agent is launched directly by the
// tmux pane command (BuildOpencodeCmd), not via a Run call.
func (h *hostIsolator) BuildRunArgs() []string {
	return nil
}

// Run is a no-op stub for host mode: the agent is launched directly in the
...
// Shutdown is a no-op for host mode: the agent process is owned by the
...
// Isolator never observes the agent process in the production flow.
func (h *hostIsolator) HasExited() (bool, int) {
	return false, 0
}

// DumpLogs is a no-op for host mode: the agent writes directly to the tmux
