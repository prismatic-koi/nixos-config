...
//
// The profile schema is flat per-role: each profile is a map from session
// role (coordinator, worker, ac, retro, investigate, review-goal,
...
//
// prism quick pr invokes `pi --print`. Model is a pi model specifier (e.g.
// "anthropic/claude-sonnet-4-6"). Unknown fields in profiles.json are
// ignored by Go's JSON decoder, so loading a profiles.json that still
// carries a `providerOrder` key (e.g. an older system rebuild) is silently
// tolerated.
...
// init(). LoadProfiles does not use it; setting it is a no-op.
...
//
// This is the spawn-time edge-case gate: a session whose role
...
// The single live carrier of provider / model / thinking is the argv
// channel: buildDirectAgentCmd (host) and appendAgentRunOverrides +
// populatePIConfig + PIInvocation (bwrap, sandbox-exec).
