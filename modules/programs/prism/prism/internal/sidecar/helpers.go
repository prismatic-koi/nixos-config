...
//     JSON, type mismatch, or an unknown field with DisallowUnknownFields).
//
// 413 vs 400 trade-off: when DisallowUnknownFields is enabled, a body-cap
// overflow is always distinguishable from any other decode error, because the
// runtime wraps the cap-exceeded condition in *http.MaxBytesError before the
// decoder ever sees it. The function returns 413 on cap overflow and 400 on
// every other parse error — the two are distinguishable, so it uses the more
// informative status.
//
...
// such call site must justify the deviation in a code comment.
...
// Body-size caps for host-API POST handlers. 1 MiB is the
...
//
// `prism investigate`, `prism pr`, and `prism review` are included so their
// invocations surface in `prism audit` alongside the other spawn-shaped
// commands. Without them, an invoker's `prism investigate` invocation leaves
// no audit-log row, and a failed spawn (which writes durable
// session.spawn_intent / session.spawn_failed rows) has no companion
...
// second copy drifts out of sync and nothing fails. See cmd/audit.go and its
// tests.
...
// success, sometimes followed by additional output (for example, `--web`
// prints a confirmation line). The capture group is the PR number.
//
...
// prevents a partial match into a longer numeric suffix (for example,
// /pull/1234 otherwise matches as 123 with the `4` dangling).
...
// silently skips the capture. The sister write paths (backfill via
// spawn_outcome.pr_number from the merge-queue ledger) remain as fallbacks.
...
//
// The two helpers below forward to internal/authz, so the `prism checkin`
// permission predicate can reach them from `cmd/` as well as from here. These
// wrappers stay because roughly fifteen host-API handlers call them under
// these names. Forwarding keeps those call sites and their tests untouched,
// and keeps exactly one implementation of each rule. Do not reintroduce a
// body here.
...
// grant than isCoordinatorSession.
//
...
// <parent>~review-<N>-<role> (for example,
// "nixos-config@feature~review-2-review-goal") and are identifiable by the
// presence of "~review" in the session name.
//
...
// error. Delegates to db.IsSQLiteBusy. It is a package-local alias so call
// sites in this package need not change.
