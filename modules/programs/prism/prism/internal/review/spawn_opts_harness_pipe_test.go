...
// spawn_opts_harness_pipe_test.go — regression tests for the harness-pipe
// sock-path gate.
//
...
// newReviewerSpawnOpts builder. For a host-mode pi invoker the builder must
// set SpawnOpts.HarnessPipeSockPath; if it is left unset, the PI extension
// no-ops out for host-mode invokers, the `--agent` flag never registers, and
// pi rejects `--agent review-<role> --prompt "..."` as `Unknown options`.
//
...
// simultaneously — the single-builder shape collapses what
// would otherwise be four separate test surfaces (sync + async × pos +
// neg) into the three below.
//
// Test-suite isolation contract (AGENTS.md): these tests
...
// that when HarnessName="pi" (socket-pipe transport) and
// IsolationMode="host", the builder pre-computes SpawnOpts.HarnessPipeSockPath
// via session.SidecarHarnessPipePath(AgentSession).
//
// Without this, HarnessPipeSockPath stays empty for host-mode review invokers,
// the PI extension no-ops, and pi rejects --agent/--prompt as unknown flags
// — leaving every review agent idle forever.
...
// proof and mirrors the discipline used for the cmd/investigate.go gate.
