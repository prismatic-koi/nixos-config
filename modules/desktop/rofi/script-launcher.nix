...
}:
{
  config = lib.mkIf (config.nx.desktop.rofi.enable && pkgs.stdenv.isLinux) {
    home-manager.users.${config.nx.username}.home.file.".local/scripts/application.scripts.launcher" =
      let
        homeDir = "/home/${config.nx.username}";
      in
      {
        executable = true;
        text =
          ...
          ''
            ...
            # get list of scripts in ${homeDir}/.local/scripts
            scripts=$(ls ${homeDir}/.local/scripts)
            ...
            if [[ -n $selected_script ]]; then
                bash "${homeDir}/.local/scripts/$selected_script"
            fi
          '';
      };
  };
