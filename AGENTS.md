...
This catches build/test failures fast. The full homeless-shelter signal is then exercised by CI on the PR. `gofmt -l .` matches the `gofmt check` step in `pr-gate.yml` (added in #2282) — list mode (`-l`) shows what changes without mutating; once you're ready to fix any reported files, re-run with `-w .`.
